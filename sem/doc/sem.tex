...
ignál v rozsahu $u \in [-10; 10]$.  Dále znám i hodnotu deadzone - vozík
nereaguje na napětí s hodnotou $\mathrm{abs}(u)\leq0.2$.
...
x a cílem vozíku je se do něj dostat. Nejdříve budu polohu kyvadla zcela 
ignorovat, následně se pokusím tlumit jeho kmity.

\subsection{Zapojení v Simulinku}
Pro zprovoznění simulace jsem nejdříve zapojil zapojit shcéma v Simulinku
...
0 \\
\end{bmatrix}.$  
Zajímá mě totiž pouze poloha a úhel, tedy druhá a čtvrtá složka vektoru vstupu.
...
pokud se nachází v deadzone. Tyto bloky jsou velmi důležité, bez nich se
linearizovaný a nelinearizovaný model rozdělí a regulace nebude funguvoat,
viz~\ref{fig:Without_sat_and_deadzone_in}
a~\ref{fig:Without_sat_and_deadzone_out}. Po přidání těchto bloků jsou
nelineární i lineární signál během řízení v dolní poloze téměř shodné.
...
regulátoru, že se kyvadlo neustálí na cílové hodnotě, jako na 
obrázku~\ref{fig:P_pos_inf_error}. Tento jev je navíc výrazně podpořen 
existencí pásma necitlivosti, které dělá tento problém neřešitelným.
...
menší než $15 \%$ a dobu ustálení na $90 \%$ reference za $11 \mathrm{s}$.
...
Zatím jsem měl jako referenci nastavenou konstantu, jednalo se vlastně o jednotkový
skok. Pro lepší řízení mohu ovšem referenci postupně načítat (pomocí přenosu) a tím
se vyhnout zbytečně velkému překmitu. Za blok target připojím přenos druhého stupně,
jehož hodnoty odladím a dostanu tak efektivnější řízení vozíku. Pro přenos
$\frac{1}{0.05s^2 + s + 1}$ dostávám překmit $5 \%$ a v rozmezí $\pm 5 \%$ od
...
odlatit pomocí programu \texttt{rltool}, pro D regulátor umístím jednu nulu do
bodu 0 a budu hýbat s póly, dokud nezískám otpimální regulátor na
obrázku~\ref{fig:rltool_phi}.

\begin{figure}[H]
  \centering
  \includegraphics[width=0.6\textwidth]{img/rltool_phi.png}
  \caption{Hlednání paramterů D regulátoru pomocí rltool}\label{fig:rltool_phi}
\end{figure}

Hodntou jsem odladil na $k_d = 7.4102$. Regulátorem dojde k úplnému utlumení
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.6\textwidth]{img/oscillation_without_regulator.png}
  \caption{Kmitání bez regulátoru}\label{fig:oscillation_without}
\end{figure}

\begin{figure}[H]
  \centering
  \includegraphics[width=0.6\textwidth]{img/oscillation_with_regulator.png}
  \caption{Kmitání s regulátorem}\label{fig:oscillation_with}
\end{figure}

Přidáním P regulátoru bych sice dosáhlu zmenšení prvního kmitu o $\sim 20 \%$, ale
jinak PD regulátor nepřináší žádné výrazné zlepšení, proto ho nebudu
implementovat. Vyvýjím tak totiž zbytečně velkou sílu na vozík.
...
Nejdříve je nutné identifikovat systém. Přípustné hodnoty vstupního napětí jsou
v rozsahu $u \in [-1, 1]$, ale reálné napětí je desetkrát větší, tedy $u \in
[-10, 10] \mathrm{[V]}$. Deadzone jsem postupným zvyšováním napětí (viz obrázek
TODO doplnit) identifikoval na hodnotu 0.14.
