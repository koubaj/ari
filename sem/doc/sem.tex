...
vzdáleností od konce kyvaldla po jeho těžiště. Dále budu pokračovat s údaji z
...
\mathrm{m}$. Hmotnost kyvadla jsem zvážil kuchyňskou váhodu jako $m=0.157
\mathrm{kg}$ a hmotnost vozízku jako $M=0.55 \mathrm{Kg}$.

Dále jsem potřeboval zjistit moment hybnosti kyvadla. Nechal zabrzdil jsem vozík
...
\frac{mgl_{cm}}{\omega^2} = 5.64\cdot10^{-4} \mathrm{kg}\cdot\mathrm{m}^{-2}$.
...
vydělím $5$: $\Lambda =
\frac{1}{5}\mathrm{ln}\left(\frac{\Lambda_1}{\Lambda_5}\right) =
\frac{1}{5}\mathrm{ln}\left(\frac{1.655}{1.215}\right) = 0.174$
...
provést přechodový děj - do kyvadla pustím konstantní napětí $U_{in} = 4
\mathrm{V}$ po dobu $1.6 \mathrm{s}$. Zjistím čas $\tau=0.26\mathrm{s}$, za
který kyvadlo dosáhne $63\%$ rychlosti. Konstantu tření vozíku spočítám jako $b
...
představující kyvadlo, takže pomocí pomocí nástroje \texttt{rltool} odladím
...
Pro obě řešení jsem rovnou zvolil metodu s integrálním řešením a s pozorovatelem, protže jsou nejefektivnější.
...
póly pro stavové řízení s integrální složkou: "$-9+11.4508i$, $-9-11.4508i$,
